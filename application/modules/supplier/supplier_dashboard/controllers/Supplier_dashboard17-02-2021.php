...
public function updatePassword()
{
	...
	public function insert_companyprofile(){ 
        ...
        $res='';
			...
			$data1['username']=$this->input->post('mobilenumber');
			$data1['password']=$this->input->post('password');
			$data1['user_type']=$this->input->post('usertype');
            ...
            else
            {
             ...
             else
             {
             	$res=  $this->dashM->insert('employees',$data);
             	       $this->dashM->insert('suppliers',$data1);
             	 if($res>=0)
            {
	...
	public function getEmployeesListData() {
	 ...
	 $edit ='<a href="javascript:void(0);"> <span style="color:red"><i class="fa fa-pencil" id="edit-supplier" aria-hidden="true"></i></span></a>';
			...
			if($r->islocked==true)
			{
			$status='<a href="javascript:void(0);"><span style="color:red"><i class="fa fa-lock" id="lock-supplier" aria-hidden="true"></i></span></a>';
			}
			else
			{
				$status='<a href="javascript:void(0);"><span style="color:red"><i class="fa fa-unlock" id="lock-supplier" aria-hidden="true"></i></span></a>';
			}
                  $data[] = array(
							'no'=>$i,
							'id'=>$r->id,
  ...
  //       $data['getemployeesdetail']=$this->dashM->getemployeesdetail($id);
   ...
   //          echo ' <script>window.location.href="";</script>';
			$this->session->set_flashdata('error','You are already registerd. Please Login');
