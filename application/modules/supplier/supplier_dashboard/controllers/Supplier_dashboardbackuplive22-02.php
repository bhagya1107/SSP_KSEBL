...
public function updatePassword()
{
	...
	public function getEmployeesListData() {
	 ...
	 $edit ='<a href="javascript:void(0);"> <span style="color:red"><i class="fa fa-pencil" id="edit-supplier" aria-hidden="true"></i></span></a>';
			...
			if($r->islocked==true)
			{
			$status='<a href="javascript:void(0);"><span style="color:red"><i class="fa fa-lock" id="unlock-supplier" aria-hidden="true"></i></span></a>';
			}
			else
			{
...
//---------BL---------------------//
 public function check_supplier()
    {
        $supplierid = $this->input->post('material');
        $count = $this->users_model->check_suppliermaterial($supplierid);
        echo $count;
    }
//---------------------------//
