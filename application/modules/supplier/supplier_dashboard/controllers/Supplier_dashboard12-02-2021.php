...
class Supplier_dashboard extends SP_Controller { 
	...
	public function profile($tab=1)
	{
		...
		$data['indexurl'] = site_url()."supplier/dashboard";
	...
	{
		$loged_user=$_SESSION['userid'];
		$usertype=$_SESSION['user_type'];//echo $usertype;exit;
		$data['page'] = 'profile';
		$data['mainpage'] = '';
		$data['page_title'] = 'personal_profile';
		$data['title'] = 'Profile';
		//$uid=$_SESSION['uid'];//echo $uid;exit;
		$uid=$this->session->userdata('uid');//print_r($uid);exit;
		 $data['getpersonalprofile']=$this->dashM->getpersonalprofile('suppliers',$uid);
		
		//print_r($data['getpersonalprofile']);exit;
