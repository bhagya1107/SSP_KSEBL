			...
			if (!empty($data['favid'])) {
				foreach ($data['tender'] as $key => $tender1) {
					if (in_array($tender1->id, $data['favid'])) {
						...
						$data['getAllfavtender'][$tender1->id] = ['tenderid' => $tender1->id, 'tender_title' => $tender1->tender_title, 'tender_date' => $tender1->tender_date, 'tender_num' => $tender1->tender_num, 'tender_description' => $tender1->tender_description, 'is_applied' => $favappliedtenders[$tender1->id]->is_applied];
					} else {
