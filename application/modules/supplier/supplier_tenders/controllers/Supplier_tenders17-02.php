	 ...
	 //neethu
	  ...
	  public function apply($tenderId){
	   ...
	   { //neethu end
		...
		$where =" WHERE tq.tender_id = $tenderId";//print_r($where);exit;
			...
			foreach ($form_questions['question'] as $key => $value) {
				...
				if($form_questions['option_list'][$key]){
					...
					if($inputtype =='textarea'){
						$dataAppend.="<div class='form-group col-md-8'><textarea class='form-control' name='questionid'></textarea></div>
						</div>";
					}else{
						$dataAppend.="	<div class='form-group col-md-8'>
									<input type='$inputtype' class='form-control' required name='questionid'>
									</div>
							</div>";
					}
				}
